...
class PlanetApp:
    def __init__(self, root):
        ...
        self.info_button = tk.Button(self.button_frame, text="Tell me everything about", command=self.show_planet_info)
        self.info_button.grid(row=0, column=0, padx=5)

        self.mass_button = tk.Button(self.button_frame, text="Planet Size...?", command=self.show_planet_mass)
        self.mass_button.grid(row=0, column=1, padx=5)

        self.list_button = tk.Button(self.button_frame, text="check if this is a planet ?", command=self.check_planet_in_list)
        self.list_button.grid(row=0, column=2, padx=5)

        self.moon_button = tk.Button(self.button_frame, text="How many moons does this planet have?", command=self.show_moon_count)
